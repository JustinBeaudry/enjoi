...
function enjoi(schema, options = {}) {
  ...
  function resolveAnyOf(current) {
    assert(_.isArray(current.anyOf), 'Expected anyOf to be an array.');
  ...
  function resolveAllOf(current) {
    assert(_.isArray(current.allOf), 'Expected allOf to be an array.');
  ...
  function resolveOneOf(current) {
    assert(_.isArray(current.oneOf), 'Expected allOf to be an array.');
  ...
  function resolveNot(current) {
    assert(_.isArray(current.not), 'Expected Not to be an array.');
  ...
  function number(current) {
    ...
    _.isNumber(current.exclusiveMinimum) && (joischema = joischema.greater(current.exclusiveMinimum));
    _.isNumber(current.exclusiveMaximum) && (joischema = joischema.less(current.exclusiveMaximum));
    _.isNumber(current.multipleOf) && current.multipleOf !== 0 && (joischema = joischema.multiple(current.multipleOf));
